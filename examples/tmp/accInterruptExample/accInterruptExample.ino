...
//hardware interrupt on the int1 pin for the accelerometer. You can use the
...
void setup() {
  ...
  int gyroCS = 6, magCS = 7;

  Serial.print("starting, setup spi: ");
  Serial.println(device.getErrorString(device.useSPI(gyroCS, magCS)));
  ...
  Serial.println(device.getErrorString(device.setAccODR(OutputDataRate::Hz_119)));
...
void loop() {
  ...
  //We update our samples and then print them to the screen.
